...
void AddCountT2Hour(RRD * rrd, u32 rp, u32 rb, u32 tp, u32 tb)
{				/*每2天一次 */
	int n;
	n = (curt / 7200) % 180;
#ifdef DEBUG
...
void AddCountT12Min(RRD * rrd, u32 rp, u32 rb, u32 tp, u32 tb)
{				/*每12分钟一次 */
	int n;
	n = (curt / 720) % 180;
...
void AddCountMin(RRD * rrd, u32 rp, u32 rb, u32 tp, u32 tb)
{				/*每分钟一次 */
	int index;
...
void AddCount(char *devname, u32 rp, u32 rb, u32 tp, u32 tb)
{				/* 每秒钟一次 */
	u32 index, lastsec;
	u32 rp1, rb1, tp1, tb1;
...
void dostatis()
{
	...
	while (fgets(buf, MAXLEN, fp) != NULL) {
		...
		*p = 0;
		...
		while (*devname == ' ')
			devname++;
		if (strcmp(devname, "dummy0") == 0 || strcmp(devname, "tunl0") == 0 || strcmp(devname, "gre0") == 0 || strcmp(devname, "lo") == 0)
			continue;

		if (sscanf(p, "%lu %lu %lu %lu %lu %lu %lu %lu %lu %lu", &rb, &rp, &ign, &ign, &ign, &ign, &ign, &ign, &tb, &tp) != 10) {
			fprintf(stderr, "Could not open /proc/net/dev\n");
			sleep(30);
			continue;
		}
